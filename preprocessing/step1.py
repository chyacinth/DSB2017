...
def binarize_per_slice(image, spacing, intensity_th=-600, sigma=1, area_th=30, eccen_th=0.99, bg_patch_size=10):
    ...
    for i in range(image.shape[0]):
        ...
        if len(np.unique(image[i, 0:bg_patch_size, 0:bg_patch_size])) == 1:
            # img_bitmap = Image.fromarray(image[i])
            # Image._show(img_bitmap)
            # nan_mask = nan_mask * 1000
            # img_bitmap = Image.fromarray(nan_mask)
            # Image._show(img_bitmap)
...
def step1_python(case_path):
    ...
    img_bitmap = Image.fromarray(case_pixels[50])
    Image._show(img_bitmap)
...
def step1_python_mhd(case_path):
    #case_path = '/Users/hyacinth/Downloads/tianchi/val_subset00/LKDS-00121.mhd'
    case = sitk.ReadImage(case_path)
    case_pixels = sitk.GetArrayFromImage(case)
    # for i in range(case_pixels.shape[0]):
    #    case_pixels[i] = matrix2int16(case_pixels[i])
    #spacing = case.getSpacing()
    spacing = np.array(case.GetSpacing())
    img_bitmap = Image.fromarray(case_pixels[50])
    #Image._show(img_bitmap)
