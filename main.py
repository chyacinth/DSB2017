from preprocessing import full_prep,full_prep_mhd
...
if not skip_prep:
    if not use_mhd:
        ...
        img_array_npy = np.load(prep_result_path + testsplit[0] + '_clean.npy')
        print(img_array_npy.shape)
        img_bitmap = Image.fromarray(img_array_npy[0][100])
        Image._show(img_bitmap)
    else:
        testsplit = full_prep_mhd(datapath,prep_result_path,
                          n_worker = config_submit['n_worker_preprocessing'],
                          use_existing=config_submit['use_exsiting_preprocessing'])

        img_array_npy = np.load(prep_result_path + testsplit[1] + '_clean.npy')
        print(img_array_npy.shape)
        img_bitmap = Image.fromarray(img_array_npy[0][100])
        Image._show(img_bitmap)
