...
import warnings
from step1 import step1_python_mhd
...
def savenpy_mhd(id, filelist, prep_folder, data_path, use_existing=True):
    resolution = np.array([1, 1, 1])
    name = filelist[id]
    if use_existing:
        if os.path.exists(os.path.join(prep_folder, name + '_label.npy')) and os.path.exists(
            ...
            return
    try:
        im, m1, m2, spacing = step1_python_mhd(os.path.join(data_path, name))
        Mask = m1 + m2
...
def full_prep_mhd(data_path, prep_folder, n_worker=None, use_existing=True):
    warnings.filterwarnings("ignore")
    if not os.path.exists(prep_folder):
        os.mkdir(prep_folder)
    ...
    # filelist = [f for f in os.listdir(data_path)]
    filelist = []
    filelist += [each for each in os.listdir(data_path) if each.endswith('.mhd')]
    partial_savenpy = partial(savenpy_mhd, filelist=filelist, prep_folder=prep_folder,
                              data_path=data_path, use_existing=use_existing)
