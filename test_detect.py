...
def test_detect(data_loader, net, get_pbb, save_dir, config,n_gpu):
    ...
    for i_name, (data, target, coord, nzhw) in enumerate(data_loader):
        ...
        # splitlist = range(0, len(data) + 1, 1)
        ...
        for i in range(len(splitlist)-1):
            input = Variable(data[splitlist[i]:splitlist[i+1]], volatile = True).cpu() #.cuda()
            inputcoord = Variable(coord[splitlist[i]:splitlist[i+1]], volatile = True).cpu() #.cuda()
            if isfeat:
